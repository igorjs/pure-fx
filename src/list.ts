...
import { type DeepReadonly, isObjectLike, deepEqual } from './internals.js';
...
const LIST_METHOD_KEYS = new Set([
  'append', 'prepend', 'setAt', 'updateAt', 'removeAt',
  'map', 'filter', 'reduce', 'find', 'findIndex', 'at', 'first', 'last',
  'concat', 'slice', 'sortBy', 'flatMap', 'equals',
  'toMutable', 'toJSON', '$raw', '$immutable',
]);
...
const buildListMethods = <T>(raw: readonly T[]): ListMethods<T> => ({
  append(value: T) { return createListProxy([...raw, value]); },
  prepend(value: T) { return createListProxy([value, ...raw]); },
  setAt(index: number, value: T) { const c = raw.slice() as T[]; c[index] = value; return createListProxy(c); },
  updateAt(index: number, fn: (current: T) => T) { const c = raw.slice() as T[]; c[index] = fn(raw[index]!); return createListProxy(c); },
  removeAt(index: number) { const c: T[] = []; for (let i = 0; i < raw.length; i++) { if (i !== index) c.push(raw[i]!); } return createListProxy(c); },
  map<U>(fn: (value: T, index: number) => U): ImmutableList<U> { return createListProxy(raw.map(fn)); },
  filter(predicate: (value: T, index: number) => boolean) { return createListProxy(raw.filter(predicate)); },
  reduce<U>(fn: (acc: U, value: T, index: number) => U, init: U): U { return raw.reduce(fn, init); },
  find(predicate: (value: T, index: number) => boolean): Option<T> { const f = raw.find(predicate); return f === undefined ? None : Some(f); },
  findIndex(predicate: (value: T, index: number) => boolean): Option<number> { const i = raw.findIndex(predicate); return i === -1 ? None : Some(i); },
  at(index: number): Option<T> { const n = index < 0 ? raw.length + index : index; return n >= 0 && n < raw.length ? Some(raw[n]!) : None; },
  first(): Option<T> { return raw.length > 0 ? Some(raw[0]!) : None; },
  last(): Option<T> { return raw.length > 0 ? Some(raw[raw.length - 1]!) : None; },
  concat(other: ImmutableList<T> | readonly T[]) { const o = '$raw' in other ? other.$raw : other; return createListProxy([...raw, ...o] as T[]); },
  slice(start?: number, end?: number) { return createListProxy(raw.slice(start, end)); },
  sortBy(comparator: (a: T, b: T) => number) { return createListProxy((raw.slice() as T[]).sort(comparator)); },
  flatMap<U>(fn: (value: T, index: number) => readonly U[]): ImmutableList<U> { return createListProxy(raw.flatMap(fn)); },
  equals(other: ImmutableList<T>): boolean {
    const otherRaw = (other && typeof other === 'object' && '$raw' in other) ? other.$raw : other;
    return raw === otherRaw || deepEqual(raw, otherRaw);
  },
  toMutable(): T[] { return structuredClone(raw) as T[]; },
  toJSON() { return raw; },
  get $raw() { return raw as ReadonlyArray<DeepReadonly<T>>; },
  $immutable: true as const,
});

const LIST_HANDLER: ProxyHandler<readonly unknown[]> = {
  get(target, prop, receiver) {
    if (typeof prop === 'string' && LIST_METHOD_KEYS.has(prop)) return (LIST_METHODS.get(target) as any)?.[prop];
    if (typeof prop === 'string' && prop.length > 0 && prop.charCodeAt(0) >= 48 && prop.charCodeAt(0) <= 57) {
      ...
      if (Number.isInteger(idx) && idx >= 0 && idx < target.length) {
        ...
        if (isObjectLike(val)) {
          ...
          if (idxMap === undefined) { idxMap = new Map(); LIST_CHILD_CACHE.set(target, idxMap); }
          let cached = idxMap.get(idx);
          if (cached === undefined) { cached = createRecord(val as object); idxMap.set(idx, cached); }
          return cached;
        }
        return val;
      }
  ...
  set(_, prop) { throw new TypeError(`Cannot set '${String(prop)}' on immutable list`); },
  deleteProperty(_, prop) { throw new TypeError(`Cannot delete '${String(prop)}' on immutable list`); },
  defineProperty(_, prop) { throw new TypeError(`Cannot define '${String(prop)}' on immutable list`); },
};
